...
test("GET deliveries: returns campaign sends newest-first, filterable by channel/status", async () => {
  ...
  try {
    ...
    const campaign = await prisma.voiceCampaign.create({
      data: { tenantId, name: "Feed", status: "active", channels: JSON.stringify(["whatsapp", "email"]) },
    });
    const lead = await prisma.campaignLead.create({ data: { campaignId: campaign.id, tenantId, firstName: "A", phone: "+919000000001", consent: true } });

    // Three deliveries across channels/statuses; createdAt ordering controlled explicitly.
    await prisma.messageDelivery.create({ data: { tenantId, campaignId: campaign.id, leadId: lead.id, channel: "whatsapp", status: "sent", renderedBody: "hi", createdAt: new Date(Date.now() - 3000) } });
    await prisma.messageDelivery.create({ data: { tenantId, campaignId: campaign.id, leadId: lead.id, channel: "email", status: "failed", error: "bounced", createdAt: new Date(Date.now() - 2000) } });
    await prisma.messageDelivery.create({ data: { tenantId, campaignId: campaign.id, leadId: lead.id, channel: "whatsapp", status: "delivered", createdAt: new Date(Date.now() - 1000) } });
...
test("GET deliveries: a campaign in another tenant is 404 (no cross-tenant leak)", async () => {
  ...
  try {
    ...
    const campaignA = await prisma.voiceCampaign.create({ data: { tenantId: hotelA, name: "A", status: "active", channels: JSON.stringify(["whatsapp"]) } });
