...
export function renderQuotationHtml(data: QuotationPdfData): string {
  ...
  // Tax summary (right column of the footer).
...
<style>
  ...
  *{ box-sizing:border-box; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
  ...
  body{ font-family:var(--sans); color:var(--ink); font-size:12.5px; line-height:1.45; -webkit-font-smoothing:antialiased; }
  .sheet{ position:relative; }

  .head{ display:grid; grid-template-columns:1.05fr 1px .95fr; gap:22px; }
  .head .rule{ background:var(--hair); }
  .brand{ display:flex; gap:12px; align-items:flex-start; }
  .brand .mark{ width:46px; height:46px; flex:none; border-radius:9px;
    background:linear-gradient(150deg,var(--accent),var(--accent-d)); display:flex; align-items:center; justify-content:center; }
  .brand .mark span{ font-family:var(--serif); color:var(--on-accent); font-size:25px; line-height:1; font-weight:700; }
  .brand .mark-img{ background:#fff; border:1px solid var(--hair); padding:5px; }
  .brand .mark-img img{ max-width:100%; max-height:100%; object-fit:contain; }
  .brand h1{ margin:0; font-family:var(--serif); font-weight:700; font-size:24px; letter-spacing:.01em; color:var(--accent); line-height:1.05; }
  .seller-lines{ margin-top:14px; display:grid; gap:3px; font-size:12px; color:var(--muted); }
  .seller-lines .k{ color:var(--ink); font-weight:600; }
  .doc-title{ font-family:var(--serif); font-size:31px; letter-spacing:.14em; color:var(--ink); margin:2px 0 16px; font-weight:700; }
  .meta{ display:grid; grid-template-columns:auto 1fr; gap:6px 12px; font-size:12px; margin:0; }
  ...
  .banner{ background:linear-gradient(180deg,var(--accent),var(--accent-d)); color:var(--on-accent);
    border-radius:5px; padding:8px 13px; font-size:11px; letter-spacing:.16em; text-transform:uppercase; font-weight:650; }
  .billto{ margin-top:22px; }
  .billto .who{ margin-top:12px; font-size:15px; font-weight:700; }
  .billto .lines{ margin-top:4px; font-size:12px; color:var(--muted); }
  .sep{ color:var(--hair); margin:0 8px; }

  .items{ margin-top:20px; }
  table{ width:100%; border-collapse:collapse; margin-top:10px; border:1px solid var(--hair); border-radius:6px; overflow:hidden; font-size:12.5px; }
  thead{ display:table-header-group; }
  thead th{ background:linear-gradient(180deg,var(--accent),var(--accent-d)); color:var(--on-accent);
    font-weight:600; letter-spacing:.04em; padding:9px 12px; text-align:right; white-space:nowrap; }
  thead th.l{ text-align:left; }
  tbody td{ padding:11px 12px; border-bottom:1px solid var(--hair-2); vertical-align:top; text-align:right; font-variant-numeric:tabular-nums; }
  tbody td.l{ text-align:left; }
  tbody tr:last-child td{ border-bottom:none; }
  tr{ break-inside:avoid; }
  .piece{ font-weight:700; font-size:13px; }
  ...
  .thumb{ width:38px; height:30px; border-radius:4px; border:1px solid var(--hair); object-fit:cover; background:var(--panel); }
  .num-strong{ font-weight:700; }

  .subtotal-bar{ display:grid; grid-template-columns:1fr auto auto; gap:18px; align-items:center; margin-top:10px;
    padding:10px 14px; border-radius:6px; background:linear-gradient(180deg,var(--accent),var(--accent-d));
    color:var(--on-accent); font-size:13px; font-weight:650; font-variant-numeric:tabular-nums; break-inside:avoid; }
  .subtotal-bar .lbl{ letter-spacing:.12em; text-transform:uppercase; font-size:11px; }
  .note{ margin-top:8px; font-size:10.5px; color:var(--faint); font-style:italic; }

  .foot{ margin-top:24px; display:grid; grid-template-columns:1fr .82fr; gap:30px; break-inside:avoid; }
  .foot h3{ margin:0 0 8px; font-size:11px; letter-spacing:.13em; text-transform:uppercase; color:var(--ink); }
  .block + .block{ margin-top:16px; }
  .kv{ font-size:12px; color:var(--muted); line-height:1.7; }
  .kv b{ color:var(--ink); font-weight:600; }
  .terms{ margin:0; padding:0; list-style:none; font-size:11.5px; color:var(--muted); line-height:1.5; }
  .terms li{ position:relative; padding-left:15px; margin-bottom:5px; }
  ...
  .summary .row{ display:flex; justify-content:space-between; gap:12px; padding:6px 0; font-variant-numeric:tabular-nums; }
  ...
  .summary .grand{ display:flex; justify-content:space-between; align-items:baseline; gap:12px; margin-top:8px;
    padding:11px 13px; border-radius:6px; background:var(--panel); border:1px solid var(--hair); font-variant-numeric:tabular-nums; }
  .summary .grand .lab{ font-family:var(--serif); font-size:14px; font-weight:700; }
  .summary .grand .val{ font-family:var(--serif); font-size:19px; font-weight:700; color:var(--accent); }
  .summary .words{ margin-top:7px; font-size:11px; color:var(--muted); font-style:italic; text-align:right; }

  .signs{ margin-top:36px; display:grid; grid-template-columns:1fr 1fr; gap:30px; align-items:end; break-inside:avoid; }
  ...
  .footer-mark{ margin-top:26px; padding-top:12px; border-top:1px solid var(--hair-2); display:flex; justify-content:space-between;
...
<body><div class="sheet">
  ...
  <section class="foot">
    ...
    <div class="right">
      <div class="summary">
        ${sumRows.join("")}
        <div class="grand"><span class="lab">Total Amount</span><span class="val">${money(v.grandTotalPaise)}</span></div>
