    ...
    <div>
      ...
      <div className="page-header">
        <div className="flex items-center gap-4">
          <Link href="/guest-database" className="p-2 rounded-lg hover:bg-slate-100 transition-colors">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {guest.segment === "VIP" && <Star className="w-4 h-4 text-amber-400 fill-amber-400" />}
            ...
            {ai?.intelligence ? (
              <div className="space-y-3">
                ...
                {ai.intelligence.attentionFlags.length > 0 && (
                  <div>
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-red-400 mb-1.5">Attention Flags</div>
                    <div className="space-y-1">
                      {ai.intelligence.attentionFlags.map((f, i) => (
                        <div key={i} className="text-xs text-red-600 flex items-start gap-1.5"><AlertCircle className="w-3 h-3 mt-0.5 shrink-0" />{f}</div>
                      ))}
                    </div>
                  </div>
                )}
            ...
            {guest.serviceRequests.length === 0 ? (
              ...
              <div className="space-y-2">
                {guest.serviceRequests.map((sr) => (
                  <div key={sr.id} className="flex items-start gap-3 p-3 rounded-lg border border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-slate-800 truncate">{sr.summary}</div>
              ...
              <div className="space-y-2">
                {guest.connectorEvents.map((ev) => (
                  <div key={ev.id} className="flex items-start gap-3 p-3 rounded-lg border border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm text-slate-700 truncate">{ev.aiSummary ?? "Message received"}</div>
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
