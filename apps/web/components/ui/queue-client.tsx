    ...
    <div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium border border-line rounded-lg text-fg-muted hover:bg-surface-inset"
            >
      ...
      <div className="kpi-grid mb-5">
        <div className="card" style={{ borderLeft: "3px solid #f59e0b" }}>
          ...
          <div className="text-xs text-fg-muted mt-1">active now</div>
        </div>
        <div className="card">
          <div className="kpi-label">In-Progress</div>
          <div className="kpi-value">{kpis.inProgress}</div>
          <div className="text-xs text-fg-muted mt-1 flex items-center gap-1"><RefreshCw className="w-3 h-3" /> staff assigned</div>
        </div>
        <div className="card" style={{ borderLeft: kpis.escalated > 0 ? "3px solid #ef4444" : undefined, background: kpis.escalated > 0 ? "#fff5f5" : undefined }}>
          <div className="kpi-label text-danger">Escalations</div>
          <div className="kpi-value text-danger">{kpis.escalated}</div>
          {kpis.escalated > 0 && <div className="text-xs text-danger mt-1 font-semibold uppercase tracking-wide">URGENT</div>}
        </div>
        <div className="card">
          <div className="kpi-label">Unassigned</div>
          <div className="kpi-value">{kpis.unassigned}</div>
          <div className="text-xs text-fg-muted mt-1">awaiting an owner</div>
        </div>
      </div>
        ...
        <div className="card col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              className="w-7 h-7 rounded-lg flex items-center justify-center text-fg-muted hover:bg-surface-inset transition-colors"
            >
            ...
            {["All", "open", "accepted", "escalated", "resolved"].map((s) => {
              ...
              return (
                <a key={s} href={s === "All" ? "/queue" : `/queue?status=${s}`}
                  className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${active ? "bg-accent text-accent-contrast" : "bg-surface-inset text-fg-muted hover:bg-surface-inset"}`}>
                  {label}
                </a>
              );
            })}
            ...
            <table className="data-table">
              ...
              <tbody>
                {items.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className={overdue ? "bg-danger-bg" : ""}>
                      <td>
                        <span className={priorityBadge[item.priority ?? ""] ?? "badge badge-slate"}>
                          {(item.priority ?? "—").toUpperCase()}
                        </span>
                      </td>
                      <td><span className="text-fg font-medium text-sm">{categoryLabel[item.category] ?? item.category}</span></td>
                      <td><span className="text-fg-muted text-sm truncate max-w-xs block">{item.summary}</span></td>
                      <td>
                        <span className={`font-mono text-sm flex items-center gap-1 ${overdue ? "text-danger font-bold" : "text-fg-muted"}`}>
                      ...
                      <td>
                        {item.assignedToUserId ? (
                          <span className="flex items-center gap-1.5">
                            <span className="w-6 h-6 rounded-full bg-accent flex items-center justify-center text-accent-contrast text-[10px] font-semibold">
                              {(userMap[item.assignedToUserId] ?? "?").split(" ").map(w => w[0]).join("").slice(0, 2)}
                            </span>
                            <span className="text-sm text-fg">{(userMap[item.assignedToUserId] ?? "").split(" ")[0]}</span>
                          </span>
                        ) : (
                          <span className="text-fg-muted text-sm">Unassigned</span>
                        )}
                      </td>
        ...
        <div className="flex flex-col gap-4">
          <div className="card flex-1">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-fg-muted uppercase tracking-wider">ACTIVE LOAD</span>
            </div>
            {staffWorkload.length === 0 ? (
              <p className="text-sm text-fg-muted">No requests are assigned right now.</p>
            ) : (
                ...
                {staffWorkload.map((s) => (
                  <div key={s.name}>
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-xs text-fg-muted">{s.active} active</span>
                    </div>
