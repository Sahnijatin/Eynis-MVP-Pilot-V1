...
export function IntegrationsClient({ items, statusLoaded = true }: { items: ConnectorRegistryItem[]; statusLoaded?: boolean }) {
  ...
  return (
    <div>
      ...
      {!statusLoaded && (
        <div className="mb-5 px-3 py-2.5 rounded-lg text-sm flex items-center gap-2 bg-warn-bg text-warn border border-warn-border">
      ...
      {categories.length === 0 && (
        <div className="card text-center py-10 text-sm text-fg-muted">
      ...
      {categories.map((cat) => {
        ...
        return (
          <div key={cat} className="mb-7">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-fg-muted mb-3">{list[0]?.categoryLabel ?? cat}</h2>
...
function ConnectorTile({ item, onConnect }: { item: ConnectorRegistryItem; onConnect: () => void }) {
  ...
  return (
    <div className="card flex flex-col" style={{ minHeight: 200 }}>
      ...
      <div className="font-semibold text-fg text-sm">{item.name}</div>
      <p className="text-xs text-fg-muted mt-1 leading-relaxed flex-1">{item.description}</p>
      <div className="mt-2 mb-3">
        <div className="text-[10px] font-semibold uppercase tracking-wide text-fg-muted">What it needs</div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5 mb-3">
        ...
        <div className="text-xs text-fg-muted">{item.categoryLabel}</div>
      </div>
      <p className="text-xs text-fg-muted mb-3">{item.description}</p>

      {item.requiredFields.length === 0 ? (
        <p className="text-sm text-fg-muted">This connector needs no credentials — just enable it.</p>
      ) : (
      ...
      {error && <div className="text-xs text-danger bg-danger-bg border border-danger-border rounded-lg px-3 py-2 mt-3">{error}</div>}
    </Modal>
  );
}
