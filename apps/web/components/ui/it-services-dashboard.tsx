    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="card col-span-2">
          ...
          {totalByCat === 0 ? (
            <div className="py-4 text-sm text-fg-subtle">No open tickets — signal arrives via the email/webhook/CSV intake doors.</div>
          ) : (
        ...
        <div className="card">
          <div className="flex items-center gap-2 mb-3">
            <Ticket className="w-4 h-4 text-fg-muted" />
            <h3 className="card-title mb-0">Recent Tickets</h3>
          </div>
          {recent.length === 0 ? (
            <div className="py-6 text-center text-sm text-fg-subtle">No recent tickets yet.</div>
          ) : (
            <div className="space-y-2">
              {recent.slice(0, 6).map((t) => (
                <div key={t.id} className="text-sm border-b border-line pb-2 last:border-0">
                  <div className="font-medium text-fg truncate">{t.summary}</div>
