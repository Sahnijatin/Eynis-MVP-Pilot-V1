    ...
    >
      <div>
          ...
          {loading ? (
            ...
            <>
              <label className={`${rowCls} mb-4 ${shared ? "border-teal-300 bg-teal-50" : "border-slate-200 hover:bg-slate-50"}`}>
                <input type="checkbox" checked={shared} onChange={() => setShared((v) => !v)} className="accent-teal-600" />
                <Globe className="w-3.5 h-3.5 text-slate-500" />
                <span className="text-slate-700">Everyone in workspace</span>
              </label>
