...
1. **Crawl is dependency-free** (`fetch` + an HTML→text reducer) instead of Playwright + `@mozilla/readability`. This keeps RS-1 CI/serverless-safe with **no new npm deps and no headless-browser binary**. Most company/marketing sites yield enough static HTML for synthesis. **RS-4 (Unit D) shipped the Playwright fallback** as an *optional, opt-in* enhancement: when the static fetch returns a too-thin shell (`< RESEARCH_JS_MIN_CHARS`, default 250) **and** `RESEARCH_PLAYWRIGHT_ENABLED=true`, `sources/crawl.ts` lazily/dynamically imports `playwright` and renders the page; whichever path yields more text wins, and the result feeds the same per-tenant cache. `playwright` is **not** a declared dependency (imported via a non-literal specifier), so default installs and CI are unchanged — a self-hosted operator who wants JS rendering runs `npm i playwright && npx playwright install chromium` and sets the flag. SSRF: the entry host is validated up-front (`hostIsPublic`) and in-page navigations to literal private IPs are aborted via route interception (DNS-rebinding remains a noted residual, as on the static path).
