...
| `NEXT_PUBLIC_PLATFORM_APEX_DOMAIN` | `eynis.com` | Web: the platform apex domain shown in customer-facing Settings → Domains (`<slug>.<apex>`). Build-time inlined; set to the reseller's domain on white-label deployments. |
| `PLATFORM_BRAND_NAME` | `Eynis` | Web: platform/reseller brand name for tenants without their own brand and on the shared host (`lib/platform.ts`). |
